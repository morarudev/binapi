...
/*************************************************************************************************/
    ...
    inline std::string b2a_hex(const std::uint8_t* p, std::size_t n) {
    ...
    inline std::string hmac_sha256(const char* key, std::size_t klen, const char* data, std::size_t dlen) {
    ...
    // unused for now
    inline bool verify_signature(const unsigned char* sig, std::size_t slen, const char* data, std::size_t dlen)
    {
        bool result = true;
    ...
    // unused for now
    inline std::string rsa_sha256(const char* privkeyfile, std::size_t /*pklen*/, const char* data, std::size_t dlen)
    {
...
private:
    struct impl {
        ...
        api::result<std::string>
            ...
            boost::beast::http::response_parser<boost::beast::http::string_body> parserResponse;
            parserResponse.body_limit(16 * 1024 * 1024);

            boost::beast::flat_buffer buffer;
     

            boost::beast::http::read(ssl_stream, buffer, parserResponse, ec);
            if (ec) {
                std::cerr << __MESSAGE("msg=" << ec.message()) << std::endl;
            ...
            res.v = std::move(parserResponse.get().body());
